...
def nh3_calc_g_from_load(capacity_kw, t_c, t_e):
    """คำนวณอัตราการไหล G (kg/hr) จากโหลดความเย็น (kW)"""
...
st.caption("เวอร์ชันอัปเกรด: เพิ่มเมนูเลือกรุ่นคอมเพรสเซอร์ MYCOM สแตนด์อโลน 100%")
st.caption("⚙️ Mayekawa (Thailand) Co., Ltd.")

st.markdown("---")

# 🔘 ส่วนที่ 1: เลือกวิธีระบุอัตราการไหล G (เพิ่มตัวเลือกโมเดล MYCOM)
st.subheader("📊 1. วิธีหาค่าอัตราการไหลสารทำความเย็น (Ref. Flow Rate G)")
g_mode = st.radio(
    "เลือกวิธีคำนวณ:",
    [
        "ป้อนค่าอัตราการไหล G (kg/hr) โดยตรง", 
        "คำนวณ G จากขนาดโหลดทำความเย็นหน้างาน (Cooling Capacity)",
        "คำนวณ G จากรุ่นคอมเพรสเซอร์ MYCOM (Model Selection) 🌟"
    ],
...
if g_mode == "ป้อนค่าอัตราการไหล G (kg/hr) โดยตรง":
    ...
    # 💾 ฐานข้อมูลโมเดลยอดฮิตของ MYCOM พร้อมค่า ปริมาตรกวาด (Swept Volume m³/h ที่รอบมาตรฐาน)
    MYCOM_MODELS = {
        "MYCOM 160 VMS (315 m³/h)": 315.0,
        "MYCOM 160 VMD (441 m³/h)": 441.0,
        "MYCOM 160 VLD (574 m³/h)": 574.0,
        "MYCOM 200 VSD (574 m³/h)": 574.0,
        "MYCOM 200 VMD (718 m³/h)": 718.0,
        "MYCOM 200 VLD (859 m³/h)": 859.0,
        "MYCOM 250 VSD (1120 m³/h)": 1120.0,
        "MYCOM 250 VMD (1400 m³/h)": 1400.0,
        "MYCOM 250 VLD (1680 m³/h)": 1680.0,
        "✏️ ระบุกำหนดเอง (Custom Swept Volume)": 0.0
    }
    col_comp1, col_comp2 = st.columns(2)
    with col_comp1:
        selected_model = st.selectbox("เลือกรุ่นคอมเพรสเซอร์ MYCOM:", list(MYCOM_MODELS.keys()), on_change=reset_calculation)
        if selected_model == "✏️ ระบุกำหนดเอง (Custom Swept Volume)":
            swept_volume = st.number_input("ระบุค่า Swept Volume (m³/h):", min_value=1.0, value=500.0, step=10.0, on_change=reset_calculation)
        else:
            swept_volume = MYCOM_MODELS[selected_model]
    with col_comp2:
        vol_eff = st.slider("ประสิทธิภาพเชิงปริมาตร Volumetric Efficiency (ηv) %:", min_value=50, max_value=100, value=85, step=1, on_change=reset_calculation) / 100.0
...
if st.session_state.calculated:
    ...
    if g_mode == "ป้อนค่าอัตราการไหล G (kg/hr) โดยตรง":
        ...
        # คำนวณความหนาแน่นของไอสารทำความเย็นที่ฝั่งดูด (Suction Vapor Density @ Te) ด้วยสมการเทอร์โมไดนามิกส์
        rho_suc = (1.91666667e-05 * Evap_temp**3 + 
                   2.70238095e-03 * Evap_temp**2 + 
                   1.56345238e-01 * Evap_temp + 
                   3.67285714e+00)
        # สูตรเชิงวิศวกรรม: G = ปริมาตรกวาด * ประสิทธิภาพปริมาตร * ความหนาแน่นฝั่งดูด
        G = swept_volume * vol_eff * rho_suc
    ...
    if G <= 0:
        st.error("❌ ข้อผิดพลาด: ไม่สามารถคำนวณหาค่าอัตราไหล G ได้ หรือค่าโมเดล/โหลดผิดพลาด")
        st.session_state.calculated = False
    elif HP <= LP:
        ...
        st.info(f"💡 **สภาวะคำนวณจริง:** G = {G:.2f} kg/hr | HP = {hp_g_show:.3f} {p_label} | LP = {lp_g_show:.3f} {p_label} (Y={Y:.3f}, S={S:.3f})")
        ...
        st.download_button(
            ...
            data=log_content,
            file_name=f"Yosaku_Pro_Log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
            mime="text/plain",
